...
const binarySearch = (
  ...
  if (endIndex >= startIndex) {
    ...
    const midVideoDateStart = parseTwitchTime(
      videos[midIndex].created_at,
      timezone
    ).atStartOfDay();
    const midVideoDateEnd = parseTwitchTime(
      videos[midIndex].created_at,
      timezone
    ).atEndOfDay();

    if (
      matchEndTime.isAfterOrEqualTo(midVideoDateStart) &&
      matchEndTime.isBeforeOrEqualTo(midVideoDateEnd)
    ) {
      ...
      if (
        midVideo.title.indexOf(matchType) > -1 &&
        checkTeamsInVideo(midVideo.title, matchType, teams)
      ) {
        return midVideo;
      }
      ...
      while (leftIndex >= 0) {
        ...
        let leftVidDateStart = parseTwitchTime(
          leftVid.created_at,
          timezone
        ).atStartOfDay();
        let leftVidDateEnd = parseTwitchTime(
          leftVid.created_at,
          timezone
        ).atEndOfDay();
        ...
        if (
          matchEndTime.isAfterOrEqualTo(leftVidDateStart) &&
          matchEndTime.isBeforeOrEqualTo(leftVidDateEnd)
        ) {
      ...
      while (rightIndex < videos.length) {
        ...
        let rightVidDateStart = parseTwitchTime(
          rightVid.created_at,
          timezone
        ).atStartOfDay();
        let rightVidDateEnd = parseTwitchTime(
          rightVid.created_at,
          timezone
        ).atEndOfDay();
        ...
        if (
          matchEndTime.isAfterOrEqualTo(rightVidDateStart) &&
          matchEndTime.isBeforeOrEqualTo(rightVidDateEnd)
        ) {
          // correct video creation date -> match ended on same date
...
const getPastTwitchVideo = (match, cursor) => {
  ...
  const matchEndTime = match.time.end
    ? new dates.ZonedDateTime.fromDateTime(match.time.end)
    : matchStartTime;
  ...
  cursor =
    apiResponse.pagination && apiResponse.pagination.cursor
      ? apiResponse.pagination.cursor
      : undefined;
  ...
  const lastVidTime = parseTwitchTime(
  ...
  if (
    matchEndTime.isAfterOrEqualTo(lastVidTime.atStartOfDay()) &&
    matchEndTime.isBeforeOrEqualTo(firstVidTime.atEndOfDay())
  ) {
    // binary search videos as they are sorted by time to get all 'Full Match' videos on the match date
    const matchType =
      String(match.tournamentName) === 'All-stars'
        ? 'Official All-Star Game'
        : 'Full Match';
    const videoMatch = binarySearch(
      videos,
      0,
      videos.length - 1,
      matchEndTime,
